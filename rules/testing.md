...
I write **tests before code, always**. The moment I **develop** (write or modify production
code), a test pulls it. No production code without a test that demanded it.

**How:** load and follow **my** skill **`test-first-discipline`** — and **not** a generic TDD skill
from a plugin (e.g. `everything-claude-code:tdd` / `tdd-workflow`). Mine wins.

For **back-ends / APIs / workers / services** (notably with The Hive), it is **my**
`outside-in-diamond-tdd` skill that specializes it (coarse-grained acceptance tests, Builder, Hive
perimeter) — again, to be preferred over any generic plugin TDD skill.
...
2. **Fail-first.** See the test 🔴 red **for the right reason** (an unsatisfied assertion, not a
   ...
   only mechanical guard against the tautological test, which is my dominant failure mode.
...
> **Why this replaced "TDD baby-steps" (2026-08-15, Thomas's call).** The rule used to be one test at
> a time, with test-first batches forbidden. It was measured on a real release (Kenjaku v4.9.1) and
> the relaxed mode scored **better on first pass** than the strict one had on the release before
> (91.80 % vs 84.62 / 87.74 % mutation score on brand-new files). What that dropped is not a ceremony
> around TDD, it is TDD's own thesis — the design emerging under the pressure of one example at a
> time — so the name changed with it: what survived the measurement is **test-first**, not TDD.
> The counter-evidence long quoted against this (87 % vs 51 % in an earlier audit) actually compared
> **test-first with test-after**, never step sizes.
